...
float arcbAngle;
glm::vec3 arcbAxis;
glm::vec3 lastArcbVec(0.0f, 0.0f, 1.0f);
glm::mat4 arcball1 = glm::mat4(1.0f);
glm::mat4 arcball2 = glm::mat4(1.0f);
bool firstArcBall = true;
...
int main()
{
	...
	// initialize arcball
	// ------------------
	float px, py, pz, sqsum;
	px = lastX * 2.0f / SCR_WIDTH - 1.0f;
	py = (SCR_HEIGHT - lastY) * 2.0f / SCR_HEIGHT - 1.0f;
	sqsum = px * px + py * py;
	pz = sqsum > 1.0f ? 0.0f : sqrt(1 - sqsum);
	lastArcbVec = normalize(glm::vec3(px, py, pz));
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		if (!firstArcBall)
		{
			glm::vec3 objAxis = glm::inverse(glm::mat3(view)) * arcbAxis;
			arcball1 = glm::rotate(arcball1, arcbAngle, objAxis);
			view = view * arcball1;
		}
		...
		if (!firstArcBall)
		{
			glm::vec3 objAxis = glm::inverse(glm::mat3(model)) * glm::inverse(glm::mat3(view)) * arcbAxis;
			arcball1 = glm::rotate(arcball1, arcbAngle, objAxis);
			model = model * arcball1;
		}
		*/
...
void mouse_callback(GLFWwindow* window, double xpos, double ypos)
{
	...
	*/
	...
	// arcball vector
	float vx, vy, vz, sqsum;
	vx = xpos * 2.0f / SCR_WIDTH - 1.0f;
	vy = (SCR_HEIGHT - ypos) * 2.0f / SCR_HEIGHT - 1.0f;
	sqsum = vx * vx + vy * vy;
	vz = sqsum > 1.0f ? 0.0f : sqrt(1 - sqsum);
	glm::vec3 arcbvec = normalize(glm::vec3(vx, vy, vz));

	// rotation axis and angle
	float cosv = glm::dot(lastArcbVec, arcbvec);
	if (cosv > 1.0f)	cosv = 1.0f;
	if (cosv < -1.0f)	cosv = -1.0f;

	arcbAngle = acos(cosv);
	arcbAxis = glm::cross(lastArcbVec, arcbvec);
